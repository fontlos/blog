...
<head>
    ...
    <script type="module" crossorigin src="/static/js/120aa8f8.js"></script>
    <link rel="stylesheet" href="/static/css/a14e1a22.css">
    <script>document.head.querySelectorAll('.__static_tag').forEach(element => {element.remove('your-class')})</script>
</head>
...
再次运行 `hexo serve`, 完美解决, 并且对应页面的 HTML 文件也有了这些关键标签. 我们复制 `node_modules/hexo-theme-aurora` 文件夹到我们项目里的 `themes` 文件夹, 然后在 `package.json` 里删掉这个主题就好了, 或者, 你也可以直接使用我在 Github 上 Fork 的储存库, 将依赖版本改成 `github:fontlos/hexo-theme-aurora`

只不过这种方案确实有点麻烦了, 接下来我测试一下到底能不能改善 SEO, 如果可以的话, 我或许会尝试向官方提一个 PR
