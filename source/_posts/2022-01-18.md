...
abstracts: Neovim(https://neovim.io/) 是 Vim 的重构版本, 是 Vim 在这个时代的重生, 其完美继承了 Vim 的操作方式, 集成了 Lua 引擎, 能够使用 Lua 以及各种脚本来编写插件, 并为扩展编辑器提供更大的灵活性. 在 Neovim 0.5 版本中引入了语言服务器协议 (LSP),这意味着 Neovim 可以充当 LSP 服务器 (如 rust-analyzer)的客户端, 并协助构建增强的 LSP 工具
tags:
    - Neovim
    - Lua
categories:
...
**[Neovim](https://neovim.io/)** 是 Vim 的重构版本, 是 Vim 在这个时代的重生, 其完美继承了 Vim 的操作方式, 集成了 Lua 引擎, 能够使用 Lua 以及各种脚本来编写插件, 并为扩展编辑器提供更大的灵活性. 在 Neovim 0.5 版本中引入了语言服务器协议 (LSP),这意味着 Neovim 可以充当 LSP 服务器 (如 rust-analyzer)的客户端, 并协助构建增强的 LSP 工具
